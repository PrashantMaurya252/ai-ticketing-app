...
export const createTicket=async(req,res)=>{
    try {
        ...
        console.error("error in ticket creation",error.message)
...
export const getTicket = async(req,res)=>{
    try {
        ...
        if(!ticket){
            return res.status(404).json({message:"Ticket not foud"})
        }

        return res.status(200).json(ticket)
    } catch (error) {
        console.error("error in fetching single ticket",error.message)
        return res.status(500).json({message:"Internal Server Error"})
    }
}